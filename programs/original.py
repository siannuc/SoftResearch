...
import pyrealsense2 as rs # realsense specific
import numpy as np 
import cv2
...
# variables
klick_counter = 0 # confirms number of registered mouse clicks

# click position - pixel location
ix = 0
iy = 0
...
def get_mouse_position(event, x, y, flags, param):
	...
	if event == cv2.EVENT_LBUTTONDOWN: # if mouse is clicked
		ix, iy = x, y # save x and y at moment
		klick_counter += 1 # add on click registed
		print ("button clicked")

def good_features_to_track(src_color):
	gray = cv2.cvtColor(src_color, cv2.COLOR_BGR2GRAY) # covert image from source to destination format
	corners = cv2.goodFeaturesToTrack(gray, 1, 0.01, 10) # "strong corners of image are found" 
	corners = np.int0(corners) # turn into integers
	for i in corners: 
		x_corner, y_corner = i.ravel() # flatten
...
def main():
	...
	global step_1, step_2, step_3, step_4, klick_counter,ch_poi, koord, corner_picture_0, corner_picture_1
	# start the main - try
	try:
		while True:
